...
abstract class Lxclass
{
	...
	function __construct($masterserver, $readserver, $key)
	{
		...
		// Remove the ',' character, since it is used to separate teh delete list in forms. 
		// This is a hack, but there is no other way.. If ',' is left as it is, 
		// it becomes impossible to delete this element..... (later) throw an exception. 
		// If a variable is changed silently like this, it can cause unexpected sideffects.
	...
	*/
	...
	function syncEntireObject()
	{
		...
		if ($this->isUnclean()) {
			...
			if ($this->metadbaction !== "writeonly") {
				...
				// This is used in web to get the iis is which is generated only 
	...
	static function getdbFilter($filter = null, $class)
	{
		...
		foreach ((array)$filter as $key => $val) {
			if ($key === 'sortby' || $key === 'searchstring' || $key === 'sortdir' ||
	...
	// This functions makes sure that all the objects are defined properly. 
	// This is necessary since these definitions are used to determine the actions on the object when 
	// the parent gets deleted, toggled added etc. This makes sure that we don't have children 
	// whose properties are not properly described.
	...
	function consistencyAlreadyExisting($res, $trulist, $real)
	{
		...
		if (isset($res[0]['syncserver']) && ($this->syncserver !== $res[0]['syncserver'])) {
			$string = "Server for {$this->get__table()}:{$this->nname} in the backup is {$this->syncserver} " .
				"but in the database is {$res[0]['syncserver']}. Using the one in Database\n";
	...
	function checkForConsistency($tree, $trulist, $real = false)
	{
		...
		// If it is not real, then only go through the display part. Now there is no ther guy, 
		// so there's only one loop and saves a lot of time.
	...
	function changeUsedFromParent($qp, $flag)
	{
		...
		// Improtant. This function is called in the 'was' of a child. And thus the was of this object is already over. 
		// So we need to do a write ourselves. Be careful, WHen adding sometimes a child is 'wased' before the parent, 
		// leading to a lot unexpected quandaries. Only if the parent is in updatemode should it be written to. 
		// Else it means it was when adding, and then it should be left to the normal 'was';
	...
	{
		// DO the sending only for objects that contain the contactemail; For instance, if the mailaccount or 
		// ftpuser etc is being disabled, don't unncessarily bother all the parents about the disabling.
	...
	//	dprint("hello\n");
		...
		// Important: When the priv flags are turned off or on, you need to to do the requeistie updatesubaction. 
		// Also for hardquota variables, the main value also should be impressed upon instantly.
		if ($oldv) {
			foreach ((array)$ql as $nk => $nv) {
				if (cse($nk, "_flag")) {
					// This is also done in collectquota, so should be made into a function that can be defined only once. 
					// Later...I removed it from there. NOw there is no more used variable for flags. ONly privs. 
					// Just set it and it will be done. 
		...
		foreach ((array)$cl as $k => $v) {
			...
			if (cse($v, "_o")) {
				...
				// The child object is not a strict necessary and in some cases it may not exist at all. 
				// For instance in a forwarded domain, the uuser object will not exist, nor will the mail object.
				if (!$cb) {
					continue;
				}
	...
	//	$ol[] = "a=show&o={$class}";
		...
		// Hack hack... Removing the '__title_main' from the alist. This is already present in the parent, 
		// and it will mix everything and screw up.
		unset($onl['__title_main']);

		unset($onl['action']);

		if (isset($nl['property'])) {
			foreach ($onl['property'] as $_tq) {
				$onl[] = $_tq;
			}
		}
	...
	// This function was used earlier because ddatabase had the syncserver in its nname. 
	// From now on, for the sake for transparency across clusters, main resources cannot have it.
...
class listpriv extends lxaclass
{
	...
	// the listquota is static. To get this dynamic list quota for the admin, you need to access the parent 
	// (admin) object from inside listpriv, which is actually impossible, since listpriv, being an internal object, 
	// is automatically initialiazed when the database is loaded. Anyway, currently I have just added 
	// listpriv->__parent_o = $this inside the getThisFromDb->setFromArray() in the lxdb. This is actually not a hack, 
	// and should work pretty much fine, since setFromArray is a fundamental function and called by everyone to 
...
class lxDriverClass extends Lxclass
{
	...
	function top_level_network_backup($logto = null)
	{
		$bc = $this->do_backup();
		...
		if ($this->main->getZiptype() === 'zip') {
			$res = zip_to_fileserv($bc[0], $bc[1], $logto);
		} else if ($this->main->getZiptype() === 'tar') {
			$res = tar_to_fileserv($bc[0], $bc[1], $logto);
		} else {
			$res = tgz_to_fileserv($bc[0], $bc[1], $logto);
		}
